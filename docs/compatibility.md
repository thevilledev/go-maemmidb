...
| **Differential fuzzing of the storage engine**, in go-juuri, against `hashicorp/go-immutable-radix` v1.3.1, including transactions large enough to push upstream past its 8192-entry tracking cap into its slow-notify path. | go-juuri reproduces iteration order, prefix/lower-bound/reverse seeks, longest-prefix matching and watch granularity exactly. |
